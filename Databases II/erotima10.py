...
s3=sc.sql("select categ,count(*) as cnt from cat,rating,tag where tag.userId=rating.userId and rating.movieId=tag.movieId and cat.movieId=rating.movieId and rating.rating>3.5 group by cat.categ")
s3.show()
end_time=time.time()
